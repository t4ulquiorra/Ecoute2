...
object GoogleAuthManager {
    ...
    suspend fun getAccessToken(context: Context, account: GoogleSignInAccount): String? =
        withContext(Dispatchers.IO) {
            runCatching {
                GoogleAuthUtil.getToken(context, account.account!!, "oauth2:$YOUTUBE_SCOPE")
            }.getOrNull()
        }

    @Serializable data class PlaylistItemsResponse(
    ...
    suspend fun fetchAllLikedSongs(accessToken: String): List<Song> {
